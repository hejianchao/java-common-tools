...
public class JdkEasyWrongPoint {
    ...
    public void testEnumValueOf() {
        ...
        // right
        mysql = DB_TYPE.getByTypeName("mysql");
    }
    ...
    enum DB_TYPE {
        ...
        /**
         * 用于取代 valueOf()方法；或者内部采用一个 Map 来进行查询都可以.
         */
        public static DB_TYPE getByTypeName(String typeName) {
            for (DB_TYPE type : values()) {
                if (type.typeName.equalsIgnoreCase(typeName)) {
                    return type;
                }
            }
            throw new RuntimeException(String.format("cant find typeName:%s. DB_TYPE name should be:%s",
                    typeName, Arrays.asList(values())));
        }
    }
}
